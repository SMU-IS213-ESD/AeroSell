...
def create_payment():
	...
	Expected JSON: { "order_id": int, "amount": number, "currency": str, "method": str }
	"""
	data = None
	try:
	...
	payment = Payment(order_id=order_id, amount=amt, currency=currency, method=method, status="pending")
	try:
		if stripe.api_key:
			...
			return jsonify({"client_secret": getattr(pi, "client_secret", None), "payment_id": payment.id, "transaction_id": payment.transaction_id}), 201
		else:
			db.session.rollback()
